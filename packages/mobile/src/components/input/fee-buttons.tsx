...
export const FeeButtonsInner: FunctionComponent<FeeButtonsProps> = observer(
  ({
    ...
    const renderIconTypeFee = (label: string, selected?: boolean) => {
      switch (label) {
        case 'Low':
          return <LowIcon color={!selected ? '#1E1E1E' : colors['purple-900']} size={24} />;
        case 'Average':
          return <AverageIcon color={!selected ? '#1E1E1E' : colors['purple-900']} size={24} />;
        case 'Fast':
          return <FastIcon color={!selected ? '#1E1E1E' : colors['purple-900']} size={24} />;
        default:
          return <LowIcon color={!selected ? '#1E1E1E' : colors['purple-900']} size={24} />;
      }
    };
      ...
      return (
        ...
        >
          <View style={style.flatten(['flex-row', 'justify-between'])}>
            ...
            {renderIconTypeFee(label, selected)}
          </View>
          {price ? (
            <Text
              style={style.flatten([
